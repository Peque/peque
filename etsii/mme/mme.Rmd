...
Mean error can be easily calculated:
...
As it was shown before, some special days present less demand than others:

```{r, cache = TRUE, fig.width = 10}
specialday <- function(day) {
  correction = 0
  # January 01
  if (format(day, '%m%d') == '0101')
    correction = -100
  # December 25
  if (format(day, '%m%d') == '1225')
    correction = -100
  # May 01
  if (format(day, '%m%d') == '0501')
    correction = -100
  # January 06
  if (format(day, '%m%d') == '0106')
    correction = -100
  # August 15
  if (format(day, '%m%d') == '0815')
    correction = -100
  # December 31
  if (format(day, '%m%d') == '1231')
    correction = -100
  # November 01
  if (format(day, '%m%d') == '1101')
    correction = -100
  # December 08
  if (format(day, '%m%d') == '1208')
    correction = -100
  # October 12
  if (format(day, '%m%d') == '1012')
    correction = -100
  # December 24
  if (format(day, '%m%d') == '1224')
    correction = -100
