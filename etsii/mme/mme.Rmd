...
```{r}
df_test <- subset(df, date >= strptime('01-01-2012', format = '%d-%m-%Y'))
df <- subset(df, date < strptime('01-01-2012', format = '%d-%m-%Y'))
...
Now there are other dates which repeately generate the highest errors:

```{r}
head(strftime(dates[order(-abs_diff)], format = '%B %d'), 10)
```

Those dates may be taken into account when forecasting the demand in order to reduce the forecast error.
...
## ARIMA models and forecast

### Time series of the demand minus yearly and weekly seasonal data

Although the order for $ARIMA(p,d,q)$ can be considered from ACF and PACF, in R language there is a function to find order $(p,d,q)$ automatically:

```{r}
arima_mwys <- auto.arima(ts_mwys)
arima_mwys
```

Two dataframes are created containing the yearly and weekly seasonal data:

```{r}
auxdf <- df[!duplicated(df$yearday), ]
year_season <- auxdf$demand_mws - auxdf$demand_mwys
names(year_season) <- auxdf$yearday
head(year_season)

auxdf <- df[!duplicated(df$day), ]
week_season <- auxdf$demand - auxdf$demand_mws
names(week_season) <- auxdf$day
head(week_season)
```
...
for (i in 1:nrow(df_test)) {
  s <- year_season[df_test$yearday[i]] + week_season[df_test$day[i]]
  p <- as.numeric(predict(arima_mwys, newdata = auxts, n.ahead = 1)$pred) + s
  pred <- c(pred, p)
  errs <- c(errs, df_test$demand[i] - p)
  auxts <- ts(c(auxts, df_test$demand[i]), frequency = 1)
}
par(mfrow = c(1, 1))
plot(1:366, errs, type = 'l')
...
pred <- c()
for (i in 1:nrow(df_test)) {
  p <- as.numeric(predict(arima, newdata = auxts, n.ahead = 1)$pred)
  pred <- c(pred, p)
  errs <- c(errs, df_test$demand[i] - p)
