...
ts <- ts(df$demand, frequency = 1, start = c(2004, 01))
ts_test <- ts(df$demand, frequency = 1, start = c(2012, 01))
```
...
```{r, fig.width = 10, fig.height = 10}
wts <- ts(ts, frequency = 7, start = c(2004, 01))
...
df$demand_mwys[days365] <- df$demand_mws[days365] - dec_yts$season
...
```{r, cache = TRUE, fig.width = 10}
par(mfrow = c(1, 1))
ts_mwys <- ts(df$demand_mwys, frequency = 1, start = c(2004, 01))
...
```{r}
arima <- auto.arima(ts)
predict(arima, n.ahead = 10)
```

## TODO: ARMA, AR, MA models and forecast?
